...
class QueryBuilder
{
     ...
     */
    public function where(string|\Closure $column, mixed $value = null, mixed $operator = '='): static
    {
     ...
     */
    public function orWhere(string|\Closure $column, mixed $value = null, mixed $operator = '='): static
    {
    ...
    /**
     * Insert a new row and return its integer ID.
     */
    ...
    private function addWhere(string $type, string|\Closure $column, mixed $value, mixed $operator = '='): static
    {
        ...
        // Support both where(col, op, val) and where(col, val, op) conventions
        if (is_string($value) && isset(self::ALLOWED_OPERATORS[strtoupper(trim($value))])) {
            $tempOp = $value;
            $value = $operator;
            $operator = $tempOp;
        }
