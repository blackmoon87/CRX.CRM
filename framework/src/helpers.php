...
if (!function_exists('session')) {
    function session(): \Spartan\SessionInterface
    {
        return \Spartan\Application::$app->session;
    }
}
...
if (!function_exists('csrf_token')) {
    ...
    {
        return (string)(\Spartan\Application::$app->session->get('_csrf_token') ?? '');
    }
}

if (!function_exists('user_can')) {
    /**
     * Check if current authenticated user has a permission in the active workspace.
     */
    function user_can(string $permission): bool
    {
        if (!isset(\Spartan\Application::$app) || !\Spartan\Application::$app->auth || !\Spartan\Application::$app->auth->check()) {
            return false;
        }
        $userId = (int)\Spartan\Application::$app->auth->id();
        $wsService = new \App\Services\WorkspaceService(\Spartan\Application::$app->session);
        $wsId = $wsService->getActiveWorkspaceId($userId);
        if (!$wsId) {
            return false;
        }
        return (new \App\Services\PermissionService())->can($userId, $wsId, $permission);
    }
}

if (!function_exists('active_user_role')) {
    /**
     * Get the active workspace role slug for the current user.
     */
    function active_user_role(): string
    {
        ...
        $m = (new \App\Models\Membership)->table()
