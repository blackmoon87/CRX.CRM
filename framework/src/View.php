...
class View implements ViewInterface
{
    ...
    protected function compileString(string $content): string
    {
        ...
        // Support escaped Blade tags: @{{ ... }} rendered literally as {{ ... }}
        ...
        $content = preg_replace('/@yield\s*(\((?>[^()]+|(?1))*\))/', '<?php echo $this->yieldContent$1; ?>', $content);
        $content = preg_replace_callback('/@include\s*(\((?>[^()]+|(?1))*\))/s', function($matches) {
            $args = trim(substr($matches[1], 1, -1));
            return "<?php echo \$this->include({$args}, get_defined_vars()); ?>";
        }, $content);
    ...
    private function compileInlineSection(array $m): string
    {
        ...
        return '<?php $this->sections[\'' . addslashes($name) . '\'] = ' . $value . '; ?>';
    }
